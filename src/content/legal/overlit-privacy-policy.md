...
- OverLit shows **Google AdMob ads** in the free version **on iPhone** — banner, interstitial and opt-in rewarded ads. Google's advertising SDK processes ad-request and ad-interaction data, and your IP address, which can be used to estimate a coarse location. **The Android app contains no advertising SDK at all**; see section 3.
...
The Android app also switches off the advertising identifier that Google Analytics for Firebase would otherwise collect by default, and removes the corresponding permission from the app, because nothing in that build has any use for one.
...
Nothing is sold to a data broker. No customer list, contact list or support inbox is uploaded to any advertising platform. There is no attribution partner and no analytics vendor other than the Google products named above.
...
Apple's App Privacy labels and Google Play's Data safety section use each store's own categories, which are broader and blunter than plain language. They are meant to describe the same practices as this policy, and because the two platforms genuinely differ (section 3), **the two stores' answers are not the same**.

On the **App Store**, the labels cover advertising data and product interaction from the AdMob SDK, the app's own product-interaction events described in section 7, coarse location derived from IP address, device and SDK identifiers used for ad delivery and fraud prevention, the Full Version purchase event, and the diagnostic and performance data Google's SDK reports.
...
On **Google Play**, the Data safety answers are narrower, because the Android app has no advertising SDK: no advertising data, no advertising identifier, and no data shared with an advertising network. What it does declare is the app-activity and device information Firebase Analytics and the leaderboard service handle, and the purchase state Play Billing handles.

...
Your self-declared age is never transmitted. It is stored on the device and read only there, for three things, all local: deciding which books the library shows you (section 14), deciding which books a notification may mention (section 13), and deciding whether the advertising-measurement question is one you are old enough to be asked (section 10). An earlier version of this sentence said "used only to decide which books the library shows you", which understated it.
...
Those two are what Google Ads is told about. **Google's SDK also collects housekeeping events of its own that cannot be switched off**: that a session started, roughly how long the app was in the foreground, which screen was shown, that the app or the operating system was updated, and on iOS that a subscription renewed, converted or was refunded. An earlier draft of this section said "exactly two things are recorded". That was true of what *this app* sends and false about what the library collects on its own, and the difference is too important to leave as a rounding error — so it is corrected here rather than quietly.
...
**Where consent is needed, it is not asked of younger listeners at all.** In the European Economic Area a child's own consent is not a valid basis for processing of this kind below an age each country sets for itself — 16 in Poland, where the developer is established, and never below 13 anywhere. Rather than ask a question whose answer would not count, the app does not ask it: if the age you gave at setup is under 16, or you did not give one, the question is not shown and the advertising measurement does not run for you. That age never leaves your device; it is read where it already sits. The per-book measurement in section 9 is treated the same way, because the two share a switch.

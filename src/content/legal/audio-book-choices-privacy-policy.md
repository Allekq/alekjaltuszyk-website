...
- There is no consent-management platform, no cookie banner and no advertising-consent flow in the app. The one-time legal acceptance screen shown before you start listening is an acknowledgement of these documents, not a tracking-consent flow. There is one genuine consent in the app, and it is the milestone reporting described in this section: in the EEA, the UK, Switzerland and Canada the app asks you outright, with nothing preselected and both answers equally easy to give, before it reports anything or writes the on-device record that drives it. Everywhere else it is on by default and Settings turns it off. Nothing else in the app is optional in that sense, and nothing that is switched off is switched on again by using the app.
...
- **Withdraw consent** — where consent applies. There are two such places. The first is the milestone reporting in section 9, which is asked for outright in the EEA, the UK, Switzerland and Canada; turn it off under "Share anonymous usage data" in **Settings**, at any time, and the app stops sending immediately. Withdrawing it does not affect anything reported before you withdrew, because those events were added to per-book totals that hold no record of who caused them. The second is the iOS motion permission for the head-nod feature in section 11; turn it off in the app or in iOS Settings. Neither withdrawal costs you access to anything you have bought.

...
- Server-side, the app stores what you have bought or unlocked, how many free unlocks you have used, and any star ratings you submit. Those records are held in a Google Cloud database hosted in the United States, under the transfer safeguards described in section 8.
...
**Where these records are held.** The server code that reads and writes them runs in Google's `europe-west1` region in Belgium, but the database itself — Google Cloud Firestore — is hosted in a **United States** multi-region, and the operational logs those functions produce are held on the same US-based Google infrastructure. So the records listed in this section are stored in the United States, not in the EU. Section 8 explains the safeguards that apply to that transfer.
...
If you are in the EEA, the UK or Switzerland, your data is transferred to the United States. This is not incidental, and it is worth being clear about it.

**What goes where.** The server code that handles the app's requests runs in Google's `europe-west1` region in Belgium. The database it writes to, Google Cloud Firestore, is hosted in a United States multi-region, as are the operational logs. Everything listed in section 5 — the anonymous identifier, entitlement and subscription state, purchased book IDs, store transaction identifiers, free-unlock usage and star ratings — is therefore stored in the United States on an ongoing basis, not merely accessed from there. RevenueCat is a United States company and holds its subscriber records there. Cloudflare is a United States company operating a global network, and it sees your IP address wherever it serves a file from.

**The safeguards relied on.** These transfers are made under Article 46 GDPR safeguards and, where applicable, the European Commission's adequacy decision for the EU–US Data Privacy Framework:
...
**European Economic Area, United Kingdom, Switzerland.** Sections 6, 8 and 15 are the operative ones: legal bases, international transfers, and your rights. Section 8 matters in particular, because the server-side records described in section 5 are stored in the United States. The controller is Aleksander Jałtuszyk, Poland. The lead supervisory authority is UODO, and UK users may also contact the ICO.

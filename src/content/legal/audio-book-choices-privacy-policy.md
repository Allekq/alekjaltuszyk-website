...
Your self-declared age is never transmitted. It is stored on the device and used only to decide which books the library shows you. See section 14.
...
**The United Kingdom is in that list now and was not before, and the change deserves an explanation rather than a silent edit.** UK law exempts storage whose *sole* purpose is collecting statistics about how a service is used in order to improve it, and for as long as this switch governed only the milestone counters, that is exactly what it was. Measuring advertising is not that, and the exemption is not divisible — a second purpose sharing the same switch takes the whole of it outside. So UK listeners are now asked the same question as listeners in the EEA. This is the app giving up an exemption it genuinely had, because it stopped being entitled to it.
...
**Exactly two things are recorded, and neither is written by this app.**
...
Both are collected automatically by Google's SDK. AudioChoices logs no events of its own, and there is deliberately no code anywhere in the app that describes what you do inside a story to anyone. Nothing about which books you open, which choices you make, how long you listen or which endings you reach is sent through this, and the moment that changed this section would have to change with it.

**What identifies you, and what does not.** Google's SDK creates an **app instance ID** — a random identifier for this installation, generated on your device. It is not your name, not an email address, and not connected to the anonymous identifier in section 3. Alongside an event it also sends ordinary technical facts: device model, operating system version, app version, language, and a country worked out from your IP address. A rough location at country level is the most precise place information involved.
...
**This runs on your permission, and it is the permission you have already been asked for.** The switch is the same one described in section 9 — **Settings → "Share anonymous usage data"** — and there is deliberately not a second one. In the European Economic Area, Switzerland and Canada the app asks outright before anything is collected, and until you answer, Google's SDK is not running at all. Everywhere else it starts on, and that switch turns it off immediately.
...
**The two advertising-measurement events in section 10** are held by Google Analytics under that property's own data-retention setting, which is configured to the shortest period Google offers for user-level data — **2 months** — after which what remains is aggregate reporting that describes no individual installation. Turning off "Share anonymous usage data" stops any further events at once.
...
**Brazil, Canada, Australia, Japan, South Korea and elsewhere.** The same GDPR-grade protections and the same rights described in section 17 are applied everywhere, rather than a different standard per country. Requests go to the same address.

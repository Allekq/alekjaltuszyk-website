...
export const audioBookChoicesConfig = {
  ...
  defaultDescription:
    "AudioChoices is an interactive 'choose your story' audiobook app for iPhone and Android by Alek Jałtuszyk. Steer narrated stories with Yes/No choices by tapping or using supported headphones.",
  ...
  aiDiscovery: {
    ...
    status:
      "AudioChoices is released on the App Store for iPhone and on Google Play for Android. The public surface is a product page with a playable sample of the choice interaction, a scroll-drawn story tree, a support page, privacy policy, terms of use, and legal manifest under /apps/AudioBookChoices/. The app has no sign-up and asks for no name, email, or phone number, but it does create an anonymous installation identifier so purchases, free-unlock usage, and star ratings can be stored server-side; listening progress, settings, and listener age stay on the device. It uses Google Analytics for Firebase to measure first opens and purchases through the same Settings-controlled usage switch as its anonymous per-book milestones. It has no crash reporting, in-app advertising, advertising identifier collection, or cross-app tracking. The per-book milestones — started, reached an ending, heard a quarter/half/four fifths/all of the scenes, listened 10/30/60 minutes — go to the developer's own server and are added to per-book totals with no record of which installation caused which one; in the EEA, the UK, Switzerland, and Canada the app asks before sending either kind of usage data. Requests carry an app-integrity check (App Attest on iOS, Play Integrity on Android) so paid books cannot be pulled out in bulk. On iOS it optionally uses on-device headphone motion data for the head-nod feature. Purchases and subscriptions are handled by the App Store and Google Play, with RevenueCat as the purchase processor.",

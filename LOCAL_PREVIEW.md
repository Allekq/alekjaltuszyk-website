...
This root-level script starts the Astro dev server if needed and opens the
homepage for you.
...
That file is a tiny root-level entrypoint that launches the same local preview
flow, but is easier to spot and run directly from an editor.

## Notes

- The site is static and does not require a backend.
- Internal links are base-path-safe through `src/lib/paths.ts`.
- The root homepage is `/`, not `/apps/`.
- If port `4321` is already taken, Astro may choose another port and print the
  updated URL in the terminal.
- If you change public page structure or legal/discovery files, run:

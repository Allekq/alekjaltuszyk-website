...
def wait_for_url_and_stream_output(
    process: subprocess.Popen[str], should_open_browser: bool
) -> int:
    opened_url = False
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Start the local Astro site and open it in a browser."
    )
    parser.add_argument(
        "--host", default=DEFAULT_HOST, help="Host for Astro dev. Default: 127.0.0.1"
    )
    parser.add_argument(
        "--no-browser",
        action="store_true",
